...
export default function BootGate({ children }) {
  ...
  useEffect(() => {
    const bootTimer = setTimeout(() => setIsBooting(false), 1800)
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isBooting && (
          <motion.div
            className="fixed inset-0 z-[120] flex h-screen flex-col items-center justify-center overflow-hidden bg-white text-slate-900"
            initial={{ opacity: 1 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
