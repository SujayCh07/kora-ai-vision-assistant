    ...
    def __init__(
        ...
        *,
        ...
        self._stopper = recognizer.listen_in_background(
            microphone,
            lambda _, audio: self._callback(audio),
            phrase_time_limit=phrase_time_limit,
        )

    def _callback(self, audio: sr.AudioData) -> None:
        try:
            wav_bytes = audio.get_wav_data()
            audio_b64 = base64.b64encode(wav_bytes).decode("utf-8")
            self._queue.put(audio_b64)
            print("\n[VOICE] Captured user speech.")
...
def main() -> None:
    ...
    try:
        while True:
            ...
            if (speech_audio or should_refresh) and metadata is not None:
                response = run_cycle(
                    frame=frame,
                    metadata=metadata,
                    environment=environment,
                    ...
                    eleven=eleven,
                    snowflake=snowflake,
                    synthesize_voice=args.voice and bool(speech_audio),
                )
